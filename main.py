...
bg_image = bg_image.resize((1710, 1190))  # תמונה לגודל מלא
...
# יצירת ווידג'טים בעיצוב קליר
def clear_label(master, text):
    return tk.Label(master, text=text, bg=root["bg"], fg='black', font=("Arial", 10))
...
def draw_graph():
    ...
    try:
        ...
        messagebox.showerror("שגיאה", f"המסלול {track} לא קיים בעונה {year} או שיש בעיה בטעינה")
        return

    drivers_in_session = session.laps['Driver'].unique()
    if driver_code not in drivers_in_session:
        from tkinter import messagebox
        messagebox.showerror("שגיאה", f"הנהג {driver_code} לא השתתף במקצה הזה")
        return
...
main_frame.place(relx=0.5, rely=0.02, anchor='n')  # הרמה של הכפתורים
